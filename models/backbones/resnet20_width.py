...
class ResNet(nn.Module):
    def __init__(self, block, layers, k=16):
        self.inplanes = 1 * k
        super(ResNet, self).__init__()
        self.conv1 = nn.Conv2d(3, 1 * k, kernel_size=3, stride=1, padding=1, bias=False)
        self.bn1 = nn.BatchNorm2d(1 * k)
        self.relu = nn.ReLU(inplace=True)
        self.layer1 = self._make_layer(block, 1 * k, layers[0])
        self.layer2 = self._make_layer(block, 2 * k, layers[1], stride=2)
        self.layer3 = self._make_layer(block, 4 * k, layers[2], stride=2, last_phase=True)
        self.avgpool = nn.AvgPool2d(8, stride=1)
        self.out_dim = 4 * k * block.expansion
...
def ResNet20(width=32):
    model = ResNet(BasicBlock, [3, 3, 3], k=width)
    return model